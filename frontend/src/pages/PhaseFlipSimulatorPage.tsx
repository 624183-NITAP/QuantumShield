...
const BACKEND_URL = 'http://127.0.0.1:8000/phaseflip'

export function PhaseFlipSimulatorPage() {
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-[28px] border border-white/10 bg-slate-900/70 p-8 shadow-2xl shadow-indigo-950/20 backdrop-blur">
        <h1 className="text-3xl font-semibold text-white sm:text-4xl">Phase Flip Simulator</h1>
        <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-300">
          Run the live Qiskit phase-flip correction simulation and inspect the measured outcomes, fidelity, and recovery steps.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-[24px] border border-white/10 bg-slate-900/70 p-6 backdrop-blur">
          ...
          <p className="mt-4 text-sm leading-8 text-slate-300">
            The phase-flip code rotates the encoded state into a basis where Z-type disturbances can be detected and corrected through a parity check.
          </p>

          <div className="mt-6 rounded-[20px] border border-dashed border-indigo-400/30 bg-indigo-500/10 p-6 text-center">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              ...
              disabled={loading}
              className="rounded-full bg-indigo-500 px-4 py-2 font-medium text-slate-950 transition hover:bg-indigo-400 disabled:cursor-not-allowed disabled:opacity-70"
            >
                ...
                result.steps.map((step, index) => (
                  <div key={step} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className="mt-1 h-3 w-3 rounded-full border border-indigo-400 bg-indigo-500" />
